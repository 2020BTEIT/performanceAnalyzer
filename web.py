...
def interview_recommender(course2_list):
    ...
    for c_link in course2_list:
        ...
        if c == no_of_reco2:
            break
    return rec2_course
connection = pymysql.connect(host='localhost',user='root',password='2325',db='resume')
...
def insert_data(name,email,res_score,timestamp,no_of_pages,reco_field,cand_level,skills):
    ...
    connection.commit()
def insert_data1(name,skills,work_experience,education):
    DB_table_name = 'profiles'
    insert_sql = "insert into " + DB_table_name + """
    values (0,%s,%s,%s,%s)"""
    rec_values = (name,skills,work_experience,education)
    cursor.execute(insert_sql, rec_values)
    connection.commit()
...
def run():
    ...
    activities = ["Normal User", "Admin","Generate QR","LinkedIn_Profile","Grade_predictor"]
...
# Define options for the dropdown
    ...
    if choice=="Grade_predictor":
      file_path = "dataset.csv"   
      df = pd.read_csv(file_path)

# Split the data into features (X) and target variable (y)
      X = df[['Attendance', 'Previous_Grade', 'Study_Hours']]
      y = df['Grade']
...
# Input fields for the user to enter data
      ...
      previous_grade = st.slider("Previous Year Grade", 0, 100, 70)
      study_hours = st.slider("Weekly Study Hours", 0, 10, 5)

# Predict grade based on user input
      prediction = model.predict([[attendance, previous_grade, study_hours]])

# Display the prediction
      st.subheader("Predicted Grade:")
      st.write(f"The predicted grade is: {prediction[0]:.2f}")

# Optionally, you can display additional information or metrics
# For example, root mean squared error on the test set
...
# API endpoint and API key for LinkedIn Profile Data Extraction
        ...
        api_key = 'sfBxz3JDozMNpqPiVNakwA'  # Replace with your actual API key
        headers = {'Authorization': 'Bearer ' + api_key}

# Function to extract LinkedIn profile data
        def extract_linkedin_profile_data(linkedin_profile_url, extract_skills):
        ...
        if st.button("Extract Profile Data"):
         if linkedin_profile_url:
          with st.spinner("Extracting profile data..."):
            profile_data = extract_linkedin_profile_data(linkedin_profile_url, extract_skills) 
            ...
            if profile_data:
                ...
                if 'work_experience' in profile_data:
                    ...
                    st.subheader("LinkedIn Profile Work Experience")
                    for entry in work_experience:
                        st.write(f"- {entry['title']} at {entry['company']} ({entry['duration']})")
                else:
                    st.warning("No work experience found in the LinkedIn profile data.")
                    work_experience="NULL"
                # Extract education details
                if 'education' in profile_data:
                    education = profile_data['education']
                    st.subheader("LinkedIn Profile Education")
                    for entry in education:
                        st.write(education)
                else:
                    st.warning("No education details found in the LinkedIn profile data.") 
                    education="NULL"
            else:
                st.error("Failed to extract LinkedIn profile data. Please check the URL or try again later.")
            
          insert_data1(profile_data['full_name'], str(profile_data['skills']),str(work_experience),str(education))
        else:
